...
def explore_atp_match_data():
    ...
    if df_atp_matches is not None:
        ...
        print(
            "- 'winner_entry', 'loser_entry', 'winner_seed', 'loser_seed': Often NaN, which is expected for unseeded players or non-wildcard entries. This is likely by design."
        )
        print(
            "- 'winner_ht', 'loser_ht': Missing heights are common. Imputation might be complex (e.g., average, or from other sources if available)."
        )
        print(
            "- 'minutes': Some missing match durations. Could be imputed with care (e.g., average by set count) or matches excluded from duration-specific analysis."
        )
        ...
        if valid_rank_point_matches > 0:
            print(
                f"Percentage of matches (with valid rank points) where winner had fewer rank points (potential upsets by points): { (upsets_by_rank_points / valid_rank_point_matches * 100) if valid_rank_point_matches > 0 else 0:.2f}%"
            )
        else:
...
def explore_atp_pbp_data():
    ...
    df_atp_pbp = load_and_inspect(file_path, "Aggregated ATP PBP Data", nrows=5000)  # Load a sample for PBP
    ...
    if df_atp_pbp is not None:
        ...
        if "pbp" in df_atp_pbp.columns and not df_atp_pbp["pbp"].head(100).isnull().all():
            ...
            # Games are delimited by ';', add num_sets because each set starts a new game count implicitly
            df_atp_pbp["num_games_pbp"] = pbp_series.str.count(";") + df_atp_pbp["num_sets_pbp"]
        ...
        df_atp_pbp["player_pair_pbp"] = df_atp_pbp.apply(
            lambda row: "_".join(sorted([str(row["server1_standardized"]), str(row["server2_standardized"])])), axis=1
        )
        df_atp_pbp["match_key_pbp"] = (
            df_atp_pbp["date"].dt.strftime("%Y-%m-%d") + "_" + df_atp_pbp["tny_name"].str.lower() + "_" + df_atp_pbp["player_pair_pbp"]
        )
        ...
        if os.path.exists(atp_matches_file):
            df_atp_matches_ref = pd.read_csv(
                atp_matches_file,
                usecols=["tourney_date", "tourney_name", "winner_name", "loser_name", "score"],
                nrows=10000 if __name__ != "__main__" else None,
            )  # load more if run as script
            ...
            df_atp_matches_ref["player_pair_atp"] = df_atp_matches_ref.apply(
                lambda row: "_".join(sorted([str(row["winner_name_standardized"]), str(row["loser_name_standardized"])])), axis=1
            )
            df_atp_matches_ref["match_key_atp"] = (
                df_atp_matches_ref["tourney_date"].dt.strftime("%Y-%m-%d")
                + "_"
                + df_atp_matches_ref["tourney_name"].str.lower()
                + "_"
                + df_atp_matches_ref["player_pair_atp"]
            )

            # Attempt merge
            merged_df = pd.merge(
                df_atp_pbp, df_atp_matches_ref, left_on="match_key_pbp", right_on="match_key_atp", suffixes=["_pbp", "_atp"]
            )
...
def explore_slam_match_data():
    ...
    df_slam_matches = load_and_inspect(file_path, "Aggregated Slam Match Data")

    if df_slam_matches is not None:
        print("\n--- 3.1.2. Inspecting Columns (Slam Matches) ---")
        # Already done by load_and_inspect, here we focus on categorical distributions
        categorical_slam_match_cols = [
            "slam",
            "round",
            "status",
            "event_name",
            "court_name",
        ]  # 'nation1', 'nation2' might be too many unique values for direct plotting
...
def explore_slam_points_data():
    ...
    df_slam_points = load_and_inspect(file_path, "Aggregated Slam Points Data", nrows=20000)
    ...
    if df_slam_points is not None:
        ...
        print(
            "Percentage of missing values per column (showing top 20 with missing values):\n", missing_percent[missing_percent > 0].head(20)
        )
        ...
        if "Speed_KMH" in df_slam_points.columns and pd.api.types.is_numeric_dtype(df_slam_points["Speed_KMH"]):
            ...
            if speed_total_count > 0:
                ...
                print(
                    f"Number of points with Speed_KMH == 0: {speed_zero_count} (out of {speed_total_count} non-NaN speeds, {speed_zero_percent:.2f}%)"
                )
            else:
            ...
            plt.show()
            print(
                "Discussion: Speed_KMH == 0 likely indicates missing speed data, a fault, or a non-serve point where speed isn't measured. For serve speed analysis, these should be filtered or treated as NaN."
            )
        ...
        print(
            "Other columns like PointNumber, GameNo, SetNo should follow logical sequences; outliers here would indicate structural issues."
        )

        categorical_point_cols = [
            "PointWinner",
            "PointServer",
            "SetWinner",
            "GameWinner",
...
def explore_cross_dataset_consistency(df_atp_matches, df_atp_pbp, df_slam_matches, df_slam_points):
    ...
    if "winner_name" in df_atp_matches.columns and "loser_name" in df_atp_matches.columns:
        print(
            "Sample winner names (ATP Matches):\n",
    ...
    if "player1" in df_slam_matches.columns and "player2" in df_slam_matches.columns:
        print(
            "Sample player1 names (Slam Matches):\n",
    ...
    print(
        "- Player name formats vary (e.g., 'First Last', 'Last F.', 'Last First'). Robust matching requires careful standardization (more than the basic attempt in Part 2.5). Libraries like `fuzzywuzzy` or custom logic might be needed."
    )
    ...
    print(
        "- ATP PBP to ATP Matches: Attempted using a composite key (date, tourney_name_lower, sorted_standardized_player_names). Success depends heavily on name/tournament standardization."
    )
    print(
        "  Example PBP match key (first record):",
        df_atp_pbp["match_key_pbp"].iloc[0] if "match_key_pbp" in df_atp_pbp and not df_atp_pbp.empty else "N/A",
    )

    print("- Slam Points to Slam Matches: Direct link via 'match_id'. This is generally reliable.")
    print(
        "  Example Slam Points match_id (first record):",
        df_slam_points["match_id"].iloc[0] if "match_id" in df_slam_points and not df_slam_points.empty else "N/A",
    )
    ...
    print(
        "- For example, finding all matches involving 'Roger Federer' in `df_atp_matches` around the date of the 'Wimbledon' tournament in `df_slam_matches`."
    )
    print(
        "- This often requires external tournament schedule information or very careful fuzzy matching of tournament names and date ranges."
    )
